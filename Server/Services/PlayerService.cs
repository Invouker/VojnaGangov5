...
namespace Server.Services {
	class PlayerService {
		...
		public async void LoadPlayer(Player player, bool reload = false) {
			...
			using (var connection = Connector.GetConnection()) {
				...
				if (vgPlayer == null) { 
					...
					player.TriggerEvent("player:load:data", vgPlayer.Money, vgPlayer.BankMoney, vgPlayer.PosX, vgPlayer.PosY, vgPlayer.PosZ, vgPlayer.Dimension, vgPlayer.Hp, vgPlayer.Max_hp, vgPlayer.Armour, vgPlayer.Max_armour);
        ...
        public async void InsertPlayer(Player player) {
			...
			VGPlayer vgPlayer = new VGPlayer(player.Name, GetLicense(player), 100, 100, 0, 100, 0, 0, 0, 1, 0, 0, 0, 0, 0)
			{
